...
def get_cursor_user_id(tab, max_attempts=3, retry_interval=2):
    """
        ...
        try:
            ...
            for cookie in cookies:
                if cookie.get("name") == "WorkosCursorSessionToken":
                    return cookie["value"].split("%3A%3A")[0]
...
if __name__ == "__main__":
    ...
    try:
        ...
        if sign_up_account(browser, tab):
            ...
            if token:
                logging.info(f"获取到会话令牌: {token}")
                user_id = get_cursor_user_id(tab)
                logging.info("写入数据库...")
                # 将账户 密码 token 写入到数据库中
                db.save_account(user_id, account, password, token)
                logging.info("数据写入完成...")
                # print_end_message()
            else:
